...
let lastUrl;
let applyFunction;
const queueNavigation = utils.batch();
const queuePrefetch = utils.batch();
...
const navigate = async (url, selector, opts, goBack = false, scroll = 0) => {
  ...
  if (routeMap.has(url.pathname)) {
    ...
    if (route.vnode) {
      try {
        utils.patch(currentEl, route.vnode);
      } catch (_err) {
        const el = route.html ? getEl(route.html.documentElement, selector) : utils.createElement(route.vnode);
...
exports.queueNavigation = queueNavigation;
exports.queuePrefetch = queuePrefetch;
